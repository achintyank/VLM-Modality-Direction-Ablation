...
#
# The candidate vector ablation shows accuracy drops when subtracted — but
# that alone doesn't prove the DIRECTION matters. Subtracting almost any
# vector of similar magnitude could perturb the representation enough to
# hurt the probe a little. This control checks that: generate random
# directions matched to the candidate vector's norm at each layer, run the
# exact same ablation + probe test, and compare. If the candidate vector's
# drop is meaningfully bigger than the random baseline's, that's evidence
# the specific direction matters, not just "some perturbation happened."
# ============================================================================
...
for layer_idx in CANDIDATE_LAYERS:
    ...
    for scale in ABLATION_SCALES:
        ...
        for seed in range(N_RANDOM_DIRECTIONS):
            random_vector = random_vector_like(candidate_vectors[layer_idx], seed=seed) * scale
            X_random, y_random = collect_joint_features(
                model, processor, probe_test_pairs, layer_idx, image_token_id,
                ablate_vector=random_vector,
            )
            random_acc = train_linear_probe(X_random, y_random)
