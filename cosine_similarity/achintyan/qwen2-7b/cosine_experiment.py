"""
cosine_experiment.py — cosine-similarity modality-gap experiment (Qwen2-VL-7B).
...
# ---------------------------------------------------------------------------
# Load Qwen2-VL-7B (same model the candidate vector must be built on)
# ---------------------------------------------------------------------------
...
# forward passes. d_model is 3584 on Qwen2-VL-7B and 1536 on the 2B, so building
# the vector under one MODEL_ID and ablating under the other blows up mid-run.
...
def measure_cosine_per_layer(pairs):
    ...
    per-layer curves (each a list of N_LAYERS floats, layers 1..N_LAYERS):
      ...
      "cross_centered"   mean_i cos(vision_i - mu_L, text_{i+1} - mu_L)
                  The centered control matching the headline. The DIFFERENCE
                  paired_centered - cross_centered is the semantic-alignment
                  signal: identical forward passes and identical activations, so
                  the only thing that changes is whether the caption belongs to
                  the image. It is a free preview of the mismatched-pairs
                  experiment, without re-running the model.
...
print("\nBaseline cosine per layer   [centered | cross_c | gap | paired | grandmean]:")
...
print("\nAblated cosine per layer   [centered | cross_c | gap | paired | grandmean]:")
for i, L in enumerate(range(1, N_LAYERS + 1)):
...
with open(os.path.join(HERE, "cosine_results.json"), "w") as f:
    json.dump({"model_id": MODEL_ID, "n_pairs": N_PAIRS, "layers": layers,
               "metric": "paired_centered = mean over pairs of cos(vision_i - mu_L, text_i - mu_L); paired/cross/grandmean also saved",
