...
# 7B is ~8.3B params incl. the vision tower => ~16.6 GB of weights in 16-bit.
dtype = torch.bfloat16 if (device == "cpu" or torch.cuda.is_bf16_supported()) else torch.float16  
if device == "cuda":                                                             
    _p = torch.cuda.get_device_properties(0)                                    
    print(f"{_p.name}  {_p.total_memory/1e9:.1f} GB  dtype={dtype}")           
...
seen = 0
pbar = tqdm(total=N_SAMPLES, desc="images")                                      # COLAB
for sample in df.itertuples(index=False):
    ...
    if caption_mask.sum() == 0:
        tqdm.write("  ! caption tokens not located, skipping")                   # COLAB
    ...
    if n_vis > MAX_VISION_TOKENS:
        tqdm.write(f"  ! {n_vis} vision tokens > {MAX_VISION_TOKENS}, skipping big image")   # COLAB
    ...
    seen += 1
    pbar.update(1)                                                               # COLAB
    pbar.set_postfix(                                                            # COLAB
        vision=f"{results[-1]['total_vision']:.1f}",                             # COLAB
        caption=f"{results[-1]['total_caption']:.1f}",                           # COLAB
        n_vis=results[-1]["n_vision"],                                           # COLAB
        n_cap=results[-1]["n_caption"],                                          # COLAB
    )
